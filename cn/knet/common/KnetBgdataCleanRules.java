package cn.knet.excel.imp.knetexceltool.comm;
 ...
 * 大数据--通用清洗规则
 */
public class KnetBgdataCleanRules {
 private  String[] orgtypeMainArray = new String[]{"股份有限公司", "有限责任公司", "股份合作公司", "国有企业", "独资企业", "有限合伙", "普通合伙", "私营企业"};
     ...
     * @return
     */
    public KnetBgdataKeyword cleanRules(String orgName,String orgType_old) {
        ...
        if (newestName.endsWith(")")) {
            ...
            if (!Arrays.asList(KnetBgdataCommonInit.orgtype_stripWords).contains(lastSubOrgtype.replaceAll("\\(|\\)", ""))) {
                newestName = newestName.replace(lastSubOrgtype, "");
            }
        }
        // 1.0.3 剔除无效的词
        for (String wd : KnetBgdataCommonInit.useless_forbidWords) {
         ...
         * ###########################
         */
        for (String special_part : KnetBgdataCommonInit.common_skip_words_for_clean) {
        ...
        int cntOrgtype = 0;
        for (String orgtype : KnetBgdataCommonInit.orgtype_stripWords) {
            if (orgtype.length() <= 3) {
        ...
        //3.0.1 排除省 省2
        for (String sheng : KnetBgdataCommonInit.shengList) {
        ...
        if (StringUtils.isBlank(keyword.getProvince())) {
            for (String sheng2 : KnetBgdataCommonInit.shengList_striped) {
        ...
        //3.0.2排除市 市2
        for (String city : KnetBgdataCommonInit.cityList) {
        ...
        if (StringUtils.isBlank(keyword.getCity())) {
            for (String city2 : KnetBgdataCommonInit.cityList_striped) {
        ...
        //3.0.3排除县 县2
        for (String county : KnetBgdataCommonInit.countyList) {
        ...
        if (StringUtils.isBlank(keyword.getCounty())) {
            for (String county2 : KnetBgdataCommonInit.countyList_striped) {
        ...
        if (newestName.length() >= 4) {
            ...
            for (String ids : KnetBgdataCommonInit.industry_stripWords) {
            ...
            if (cntIds < 2) {
                for (String ids_ext : KnetBgdataCommonInit.industry_stripWords_ext) {
        ...
        for(String mainType:KnetBgdataCommonInit.industry_stripWords_spetial_mix){
            if (StringUtils.isNotBlank(keyword.getShortName())) {
